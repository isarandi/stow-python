...
| `parse_options` (565-626) | `cli.py: parse_cli_options`, `_ScanState`, `_scan_long_option`, `_scan_bundled_options` and their `_apply_*` helpers; `perlcompat.py: find_long_option`, `parse_optint_value`, `take_bundled_optint`, `OptionError` | divergent_documented | Hand-written subset of Getopt::Long: bundling, unique-prefix abbreviation, ambiguity lists, `POSIXLY_CORRECT`, PAT_INT value grammar, whole-line scanning with errors-then-help-then-version precedence, per-character bundle errors and D/S/R action switching all produce Perl's wording. `parse_cli_options` is the scanner loop that classifies each argument (terminator, long option, `+n`, package, short-option bundle) and hands the shared `_ScanState` — position, action, options, package lists, error and help/version flags — to the helper for that arm, which is where an option may consume the argument that follows it. The reusable Getopt::Long machinery (name resolution, value grammar, the per-option diagnostic) lives in `perlcompat.py`, the option table `_OPTION_SPECS` in `cli.py`. Deliberate non-reproductions: `+` prefixes except `+n` (#5, #22), `-v N`/`--verbose N` value gobbling (#10), `--` termination behaviour (#11). |
