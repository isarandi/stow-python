...
| `get_ignore_regexps_from_fh` (1588-1607) | `stow.py: _parse_ignore_lines`, `_read_ignore_file`, `_get_default_global_ignore_regexps` | restructured | The filehandle-generic parser is one helper over any sequence of lines, called for the ignore file and for the built-in defaults. Byte-transparent `\n`-delimited reading, ASCII-only whitespace and comment stripping, `\#` unescape, deduplication and the hardcoded self-ignore pattern all match Perl, including an ignore file that is a directory. |
| `compile_ignore_regexps` (1609-1634) | `stow.py: _compile_ignore_patterns`; `perlcompat.py: scope_leading_flags`; `types.py: IgnorePatterns` | divergent_documented | Path-versus-segment partition, both wrappers and the empty-list guards are exact; alternation order is sorted rather than hash-random (#23); the regex dialect boundary and its guardrails are #28. |
| `compile_regexp` (1636-1642) | `perlcompat.py: compile_user_regexp`, `hoist_leading_flags`; `stow.py: _compile_option_pattern` | divergent_documented | `eval { qr// }` plus `die` becomes `re.compile` in a `try`/`except re.error`. This is the point at which Perl's regex dialect is exchanged for Python's, with `re.ASCII` character semantics, hoisted leading flag groups and loud rejection of POSIX classes (#28, #12). |
...
| `parse_options` (565-626) | `cli.py: parse_cli_options`, `_ArgumentScanner` with its `scan`, `_scan_long_option`, `_scan_bundled_options` and `_apply_*` methods; `perlcompat.py: find_long_option`, `parse_optint_value`, `take_bundled_optint`, `OptionError` | divergent_documented | Hand-written subset of Getopt::Long: bundling, unique-prefix abbreviation, ambiguity lists, `POSIXLY_CORRECT`, PAT_INT value grammar, whole-line scanning with errors-then-help-then-version precedence, per-character bundle errors and D/S/R action switching all produce Perl's wording. `parse_cli_options` builds an `_ArgumentScanner` and returns its `scan()`, which is the scanner loop that classifies each argument (terminator, long option, `+n`, package, short-option bundle) and dispatches to the method for that arm; the scanner's attributes — position, action, options, package lists, error and help/version flags — are the shared state, and the method for an arm is where an option may consume the argument that follows it. The reusable Getopt::Long machinery (name resolution, value grammar, the per-option diagnostic) lives in `perlcompat.py`, the option table `_OPTION_SPECS` in `cli.py`. Deliberate non-reproductions: `+` prefixes except `+n` (#5, #22), `-v N`/`--verbose N` value gobbling (#10), `--` termination behaviour (#11). |
