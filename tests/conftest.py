...
if not os.path.exists(PERL_STOW):
    # Fall back to a system stow, but only if it is the exact pinned
    # version: silently comparing against e.g. a 2.3.x system stow would
    # make the whole suite assert the wrong spec.
    PERL_STOW = which("stow")
    PERL_CHKSTOW = which("chkstow")
    PERL_LIB = None
    if PERL_STOW is not None:
        try:
            _fallback_version = subprocess.run(
                [PERL_STOW, "--version"], capture_output=True, text=True, timeout=30
            ).stdout
        except (OSError, subprocess.SubprocessError):
            _fallback_version = ""
        if "version 2.4.1" not in _fallback_version:
            print(
                f"NOTE: system stow at {PERL_STOW} is not version 2.4.1; "
                "oracle tests will skip. Run "
                "tests/get_gnu_stow_for_testing_identical_behavior.sh "
                "to fetch the pinned oracle.",
                file=sys.stderr,
            )
            PERL_STOW = None
            PERL_CHKSTOW = None
...
def _apply_env(run_env, env):
    """Apply an env override dict to a subprocess environment copy.

    A value of None REMOVES the variable (subprocess env dicts cannot
    express "unset" via update()), anything else is set as usual.
    """
        ...
        for root, dirs, files in os.walk(self.tmpdir, followlinks=False):
            ...
            for f in sorted(files):
                ...
                if os.path.islink(full_path):
                    ...
                    # Binary read: byte-exact content comparison; a text-mode
                    # read would conflate \r\n with \n via universal newlines
                    # and crash on non-UTF-8 content.
...
def _restore_home_and_cwd():
    """Undo cross-test leakage of HOME and the working directory.

    StowTestEnv and testutil.init_test_dirs point HOME at per-test tmp
    dirs (isolation from the real ~/.stow-global-ignore); restoring here
    keeps that from coupling test order. The direction of any leak is
    safe (a stale tmp path, never the real HOME), but order-dependent
    tests are still a defect.
    """
...
def assert_stow_match(stow_env, args, setup_func=None, env=None):
    """
...
def check_link(env, path, expected_target):
    ...
    assert actual == expected_target, (
        f"{path}: expected {expected_target}, got {actual}"
    )
...
def filter_perl_lstat_before_unlink(perl_ops):
    ...
    while i < len(perl_ops):
        ...
        if (
            op["syscall"] == "lstat"
            and i + 1 < len(perl_ops)
            and perl_ops[i + 1]["syscall"] == "unlink"
            and op["paths"] == perl_ops[i + 1]["paths"]
        ):
...
def filter_perl_stat_before_stowrc_open(perl_ops):
    """Drop Perl's extra rc-file probe stat (documented difference #7).

    For a READABLE rc file Perl runs the -r test (a stat) and then opens
    it, while Python opens directly — so Perl's sequence has one more op
    and everything after it would misalign. The missing-file shape (Perl
    stat->ENOENT with no open vs Python open->ENOENT) is one aligned op
    on each side and is accepted by is_expected_syscall_diff instead.
    """
    filtered = []
    i = 0
    while i < len(perl_ops):
        ...
        if (
            op["syscall"] == "stat"
            and op["paths"]
            and op["paths"][0].endswith(".stowrc")
            and i + 1 < len(perl_ops)
            and perl_ops[i + 1]["syscall"] == "open"
            and perl_ops[i + 1]["paths"] == op["paths"]
        ):
            i += 1
            continue
        filtered.append(op)
...
def is_expected_syscall_diff(perl_op, python_op):
    ...
    # Perl probes with stat (the -r test); Python uses open directly.
    if path.endswith(".stowrc"):
        if perl_op["syscall"] == "stat" and python_op["syscall"] == "open":
            # Missing file: both probes fail with the same errno
            if perl_op["result"] == python_op["result"]:
                return True
            # Unreadable file: Perl's stat succeeds and the -r mode check
            # (pure userspace, no syscall) says no, so Perl never opens;
            # Python's open fails with EACCES.
            if perl_op["result"] != "ENOENT" and python_op["result"] == "EACCES":
                return True
        return False
    ...
    for i in range(min(max_len, 80)):
        ...
        if perl_op == python_op:
            ...
            if perl_op:
                p = perl_op
                lines.append(
                    f"P {i + 1:3d}. {p['syscall']}({', '.join(p['paths'])}) -> {p['result']}"
                )
            if python_op:
                p = python_op
                lines.append(
                    f"Y {i + 1:3d}. {p['syscall']}({', '.join(p['paths'])}) -> {p['result']}"
                )
...
def assert_stow_match_with_fs_ops(stow_env, args, setup_func=None, env=None):
    ...
    # silently. CI asserts strace is installed on Linux, so this can only
    # happen on dev machines (or OSes) without strace. Even then, the
    # rc/stdout/stderr/tree layers must still run: only the syscall layer
    # itself is skipped, and loudly.
    if shutil.which("strace") is None:
        assert_stow_match(stow_env, args, setup_func=setup_func, env=env)
        pytest.skip(
            "strace not available - rc/stdout/stderr/tree comparison ran; "
            "only the syscall layer was skipped"
        )
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix="_perl_strace.txt", delete=False
    ) as f:
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix="_python_strace.txt", delete=False
    ) as f:
    ...
    # Difference #7: Perl's -r probe stat before opening a readable rc file
    perl_ops_filtered = filter_perl_stat_before_stowrc_open(perl_ops_filtered)
