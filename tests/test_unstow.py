...
def check_protected_dirs_skipped(stderr, dirs=None):
    ...
    for dir_name in dirs:
        assert re.search(
            r"WARNING.*skipping marked Stow directory %s" % re.escape(dir_name), stderr
...
class TestUnstowSimpleTree:
    ...
    def test_unstow_simple_tree(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            stow.process_tasks()
            assert count_conflicts(stow) == 0, "conflict count"
...
class TestUnstowFromExistingDir:
    ...
    def test_unstow_from_existing_dir(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            assert os.path.isdir("lib2"), (
                "unstow simple tree from a pre-existing directory"
            )

        run_subtests(
            "unstow a simple tree from an existing directory", test_func, stow_test_env
        )
...
class TestFoldTreeAfterUnstowing:
    ...
    def test_fold_tree_after_unstowing(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            assert os.readlink("bin3") == "../stow/pkg3a/bin3", (
...
class TestInvalidLinkNotOwnedByStow:
    ...
    def test_invalid_link_not_owned_by_stow(self, stow_test_env):
        ...
        run_subtests(
            "Existing invalid link is not owned by stow", test_func, stow_test_env
        )
...
class TestDontUnlinkUnderStowDir:
    ...
    def test_dont_unlink_under_stow_dir(self, stow_test_env):
        ...
        def test_func(stow, test_dir):
            ...
            assert len(stow.tasks) == 0, "no tasks to process when unstowing pkg7b"
            assert count_conflicts(stow) == 0, "conflict count"
...
class TestDontUnlinkUnderAnotherStowDir:
    ...
    def test_dont_unlink_under_another_stow_dir(self, stow_test_env):
        ...
        def test_func(stow, test_dir):
            ...
            assert len(stow.tasks) == 0, "no tasks to process when unstowing pkg8a"
            assert count_conflicts(stow) == 0, "conflict count"
...
class TestOverridingAlreadyStowedDocumentation:
    ...
    def test_overriding_already_stowed_documentation(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            make_path("man9/man1")
            make_link(
                "man9/man1/file9.1", "../../../stow/pkg9a/man9/man1/file9.1"
            )  # emulate stow
        ...
        run_subtests(
            ...
            test_func,
            stow_test_env,
            setup_opts={"override": ["man9", "info9"]},
            check_compat_stderr=check_stderr,
        )
...
class TestDeferringToAlreadyStowedDocumentation:
    ...
    def test_deferring_to_already_stowed_documentation(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            make_link(
                "man10/man1/file10a.1", "../../../stow/pkg10a/man10/man1/file10a.1"
            )
            ...
            make_link(
                "man10/man1/file10b.1", "../../../stow/pkg10b/man10/man1/file10b.1"
            )

            make_path("../stow/pkg10c/man10/man1")
            make_file("../stow/pkg10c/man10/man1/file10a.1")
            stow.plan_unstow(["pkg10c"])
            assert len(stow.tasks) == 0, "no tasks to process when unstowing pkg10c"
            assert count_conflicts(stow) == 0, "conflict count"
...
class TestIgnoreTempFiles:
    ...
    def test_ignore_temp_files(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            stow.process_tasks()
            assert count_conflicts(stow) == 0, "conflict count"
            assert not os.path.exists("man12/man1/file12.1"), (
                "man12/man1/file12.1 was unstowed"
            )
...
class TestUnstowAlreadyUnstowed:
    ...
    def test_unstow_already_unstowed(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            make_file("../stow/pkg12/man12/man1/file12.1")
            stow.plan_unstow(["pkg12"])
            assert len(stow.tasks) == 0, "no tasks to process when unstowing pkg12"
            assert count_conflicts(stow) == 0, "conflict count"
...
class TestUnstowNeverStowed:
    ...
    def test_unstow_never_stowed(self, stow_test_env):
        def test_func(stow, test_dir):
            ...
            target_path = (
                os.path.abspath(stow.c.target) if stow.c.target != "." else os.getcwd()
            )
...
class TestUnstowWhenCwdIsntTarget:
    ...
    def test_unstow_when_cwd_isnt_target(self, stow_test_env):
        ...
        def test_func(stow, test_dir):
            ...
            assert not os.path.exists(os.path.join(test_dir, "target/bin13")), (
                "bin13/ unstowed"
            )
...
def create_and_stow_pkg(id_, pkg):
    ...
    make_link(
        "%s-%s-only-folded" % (id_, pkg), "%s/%s-%s-only-folded" % (stow_pkg, id_, pkg)
    )
    ...
    make_link(
        "%s-%s-only-folded2" % (id_, pkg),
        "%s/%s-%s-only-folded2" % (stow_pkg, id_, pkg),
    )
    ...
    make_link(
        "%s-%s-only-unfolded/file-%s" % (id_, pkg, pkg),
    ...
    make_link(
        "%s-%s-only-unfolded2/subdir/file-%s" % (id_, pkg, pkg),
    ...
    make_link(
        "%s-shared/file-%s" % (id_, pkg),
        "../%s/%s-shared/file-%s" % (stow_pkg, id_, pkg),
    )
    ...
    make_link(
        "%s-shared2/file-%s" % (id_, pkg),
        "../%s/%s-shared2/file-%s" % (stow_pkg, id_, pkg),
    )
    make_link(
        "%s-shared2/subdir/file-%s" % (id_, pkg),
...
class TestNoFolding:
    ...
    def test_no_folding(self, stow_test_env):
        ...
        assert count_conflicts(stow) == 0, "no conflicts with --no-folding"
