...
import pytest
from testutil import join_paths


# Test data: (inputs, expected, scenario)
TEST_CASES = [
...
@pytest.mark.parametrize("inputs,expected,scenario", TEST_CASES)
def test_join_paths(inputs, expected, scenario):
    """Test join_paths with various inputs."""
    got = join_paths(*inputs)
    assert got == expected, "%s: in=%r exp=[%s] got=[%s]" % (
        scenario,
        inputs,
        expected,
        got,
