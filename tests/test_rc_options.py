...
""",
        ...
        assert options.get("dir") == "../stow", "relative -d from $PWD/.stowrc"
...
""",
        ...
        assert options.get("target") == "../target", "--target from $HOME/.stowrc"
        assert options.get("dir") == "../stow", "-d ../stow from $HOME/.stowrc"
...
class TestExpandEnvironment:
    ...
    def test_expand_home(self, test_env):
        ...
        result = expand_environment_variables("$HOME/stow", "--dir option")
    ...
    def test_undefined_var_error(self, test_env, monkeypatch):
        ...
        with pytest.raises(StowCLIError):
            expand_environment_variables("$UNDEFINED", "--foo option")
    ...
    def test_expand_var_with_underscore(self, test_env, monkeypatch):
        ...
        result = expand_environment_variables("${WITH_UNDERSCORE}", "--dir option")
...
class TestExpandTilde:
    ...
    def test_tilde_expansion_to_home(self, test_env):
        ...
        result = expand_tilde_to_homedir("~/path")
        assert result == "%s/path" % test_env["abs_test_dir"], (
            "tilde expansion to $HOME"
        )

    def test_middle_tilde_not_expanded(self, test_env):
        """~ in middle of path should not be expanded."""
        result = expand_tilde_to_homedir("/path/~/here")
    ...
    def test_escaped_tilde_after_leading_tilde(self, test_env):
        ...
        globally, so ~/x\\~y becomes $HOME/x~y.
        """
        result = expand_tilde_to_homedir("~/x\\~y")
        assert result == "%s/x~y" % test_env["abs_test_dir"], (
            "leading ~ expanded, \\~ unescaped"
        )
    ...
    def test_unknown_user_with_escaped_tilde(self, test_env):
        """~nosuchuser stays literal (perl-differences #21); \\~ still unescapes."""
...
""",
        ...
        expected_target = (
            "%s/dir with space in/file with space in" % (test_env["abs_test_dir"])
        )
