...
class TestStow:
    ...
    def test_stow_simple_tree_minimally(self, stow_test_env):
        ...
        stow.plan_stow(["pkg1"])
        stow.process_tasks()
        assert stow.conflicts == {}, "no conflicts with minimal stow"
    ...
    def test_package_dir_conflicts_with_existing_non_dir(self, stow_test_env):
        ...
        stow.plan_stow(["pkg4"])
        conflicts = stow.conflicts
        assert count_conflicts(stow) == 1
        assert re.search(
    ...
    def test_package_files_conflict_with_existing_files(self, stow_test_env):
        ...
        make_file(
            "../stow/pkg4b/bin4b/file4b",
            "bin4b/file4b - version originally in stow package",
        )

        stow.plan_stow(["pkg4b"])
        conflicts = stow.conflicts
        assert count_conflicts(stow) == 2, "conflict per file"
    ...
    def test_package_files_conflict_with_existing_directories(self, stow_test_env):
        ...
        make_path(
            "file4d"
        )  # this is a directory but named like a file to create the conflict
    ...
    def test_package_files_can_adopt_existing_versions(self, stow_test_env):
        ...
        make_file(
            "../stow/pkg4c/bin4c/file4c",
            "bin4c/file4c - version originally in stow package\n",
        )

        stow.plan_stow(["pkg4c"])
        assert count_conflicts(stow) == 0, "no conflicts with --adopt"
        assert len(stow.tasks) == 4, "two tasks per file"
        stow.process_tasks()
    ...
    def test_target_already_exists_but_not_owned_by_stow(self, stow_test_env):
        ...
        stow.plan_stow(["pkg5"])
        conflicts = stow.conflicts
        assert re.search(r"not owned by stow", conflicts["pkg5"][-1]), (
            "target already exists but is not owned by stow"
        )
    ...
    def test_target_exists_owned_by_stow_but_points_to_non_directory(
        self, stow_test_env
    ):
    ...
    def test_deferring_to_already_stowed_documentation(self, stow_test_env):
        ...
        assert len(stow.tasks) == 0, "no tasks to process"
        assert count_conflicts(stow) == 0
    ...
    def test_stow_simple_tree_minimally_when_cwd_not_target(self, stow_test_env):
        ...
        cd("../..")
        stow = new_Stow(
            dir=os.path.join(test_dir, "stow"), target=os.path.join(test_dir, "target")
        )
    ...
    def test_stow_simple_tree_minimally_with_absolute_stow_and_target_dirs(
        self, stow_test_env
    ):
    ...
    def test_stow_tree_with_no_folding_enabled(self, stow_test_env):
        ...
        def create_pkg(id_prefix, pkg):
            ...
            make_file(
                "%s/%s-%s-only-new2/subdir/%s-file-%s"
                % (stow_pkg, id_prefix, pkg, id_prefix, pkg)
            )
            make_link(
                "%s/%s-%s-only-new2/current" % (stow_pkg, id_prefix, pkg), "subdir"
            )
            ...
            make_link(
                "%s-%s-only-old" % (id_prefix, pkg),
                "%s/%s-%s-only-old" % (stow_pkg, id_prefix, pkg),
            )
            make_file(
                "%s/%s-%s-only-old/%s-file-%s"
                % (stow_pkg, id_prefix, pkg, id_prefix, pkg)
            )
            ...
            make_file(
                "%s/%s-shared2/%s-file-%s" % (stow_pkg, id_prefix, id_prefix, pkg)
            )
            make_file(
                "%s/%s-shared2/subdir-%s/%s-file-%s"
                % (stow_pkg, id_prefix, pkg, id_prefix, pkg)
            )
        ...
        def check_no_folding(pkg):
            ...
            is_link(
                "no-folding-file-%s" % pkg, "%s/no-folding-file-%s" % (stow_pkg, pkg)
            )

            # check existing folded tree is untouched
            is_link(
                "no-folding-%s-only-old" % pkg,
                "%s/no-folding-%s-only-old" % (stow_pkg, pkg),
            )

            # check newly stowed shallow tree is not folded
            is_dir_not_symlink("no-folding-%s-only-new" % pkg)
            is_link(
                "no-folding-%s-only-new/no-folding-file-%s" % (pkg, pkg),
                "../%s/no-folding-%s-only-new/no-folding-file-%s"
                % (stow_pkg, pkg, pkg),
            )
            ...
            is_link(
                "no-folding-%s-only-new2/subdir/no-folding-file-%s" % (pkg, pkg),
                "../../%s/no-folding-%s-only-new2/subdir/no-folding-file-%s"
                % (stow_pkg, pkg, pkg),
            )
            is_link(
                "no-folding-%s-only-new2/current" % pkg,
            ...
            is_link(
                "no-folding-shared/no-folding-file-%s" % pkg,
            ...
            is_link(
                "no-folding-shared2/no-folding-file-%s" % pkg,
        ...
        assert stow.conflicts == {}, "no conflicts with --no-folding"
        tasks = stow.tasks
