...
from testutil import count_conflicts, new_Stow, make_path, make_file, is_dir_not_symlink
...
class TestDocumentationExamples:
    ...
    def test_bug1_stowing_empty_dirs(self, stow_test_env):
        ...
        assert count_conflicts(stow) == 0, "no conflicts stowing empty dirs"
        assert os.path.isdir("bin1"), (
            "bug 1: stowing empty dirs - directory should remain"
        )

    def test_bug2_split_tree_folding_symlinks_from_different_stow_dirs(
        self, stow_test_env
    ):
        """
        ...
        stow1 = new_Stow(dir="stow2a")
        stow1.plan_stow(["pkg2a"])
        stow1.process_tasks()

        stow2 = new_Stow(dir="stow2b")
        stow2.plan_stow(["pkg2b"])
        stow2.process_tasks()

        assert count_conflicts(stow1) == 0, "no conflicts stowing from stow2a"
        assert count_conflicts(stow2) == 0, (
            "no conflicts splitting tree-folding symlinks"
        )
        assert os.path.isdir("bin2"), (
            "tree got split by packages from multiple stow directories"
        )
